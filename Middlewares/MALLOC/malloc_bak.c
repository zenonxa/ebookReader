...
#define EXT_MEM_START_ADDR	(0X68000000+(ATK_MD0700_LCD_WIDTH)*(ATK_MD0700_LCD_HEIGHT)*2)
...
//内存池(32字节对齐)
	...
	__align(32) uint8_t mem2base[MEM2_MAX_SIZE] __attribute__((at(EXT_MEM_START_ADDR)));			//外部SRAM内存池
#endif
...
#endif
#if USE_DYNAMIC_SRAMEX
	uint16_t mem2mapbase[MEM2_ALLOC_TABLE_SIZE] __attribute__((at(EXT_MEM_START_ADDR+MEM2_MAX_SIZE)));	//外部SRAM内存池MAP
#endif
...
#if USE_DYNAMIC_SRAMIN
	.membase[SRAMIN] = mem1base,
	.memmap[SRAMIN] = mem1mapbase,
#endif
#if USE_DYNAMIC_SRAMEX
	.membase[SRAMEX] = mem2base,
	.memmap[SRAMEX] = mem2mapbase,
#endif
	0,0,  		 				//内存管理未就绪
};
