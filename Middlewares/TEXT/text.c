...
{					
    ...
    { 
        if(!bHz)
        {
	        ...
	        {      
		        ...
		        {         
					...
					atk_md0700_show_char(x,y,*str,mapping_font_size(size),POINT_COLOR);//有效部分写入
				}					
				str++; 
		        x+=size/2; //字符,为全字的一半 
	        }
        ...
        {     
	        ...
	        if(y>(y0+height-size))break;//越界返回  						     
	        Show_Font(x, y, str, fontName, fontSize, mode); //显示这个汉字,空心显示 
	        str+=2; 
	        x+=size;//下一个汉字偏移	    
        }						 
    }   
...
void Show_Str_Mid(uint16_t x,uint16_t y,uint8_t*str, FontName fontName, FontSize fontSize, uint8_t len)
{
	...
	strlenth*=size/2;
	if(strlenth>len)
		Show_Str(x,y,ATK_MD0700_LCD_WIDTH, ATK_MD0700_LCD_HEIGHT, str, fontName, fontSize, 1);
	else
	{
		strlenth=(len-strlenth)/2;
	    Show_Str(strlenth+x,y, ATK_MD0700_LCD_WIDTH, ATK_MD0700_LCD_HEIGHT, str, fontName, fontSize, 1);
	}
}
