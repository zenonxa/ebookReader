...
void Show_Font(uint16_t x,uint16_t y,uint8_t *font,uint8_t size,uint8_t mode)
{
	...
	{   												   
		...
		for(t1=0;t1<8;t1++)
		{
			if(temp&0x80)atk_md0700_draw_point(x,y,ATK_MD0700_BLACK);
			else if(mode==0)atk_md0700_draw_point(x,y,ATK_MD0700_WHITE); 
			temp<<=1;
			y++;
			if((y-y0)==size)
			{
    ...
    { 
        if(!bHz)
        {
	        ...
	        {      
		        ...
		        else atk_md0700_show_char(x,y,*str,mapping_font_size(size),POINT_COLOR);//有效部分写入 
				str++; 
		        x+=size/2; //字符,为全字的一半 
	        }
        }else//中文 
...
void Show_Str_Mid(uint16_t x,uint16_t y,uint8_t*str,uint8_t size,uint8_t len)
{
	...
	strlenth*=size/2;
	if(strlenth>len)
		Show_Str(x,y,ATK_MD0700_LCD_WIDTH, ATK_MD0700_LCD_HEIGHT, str,size,1);
	else
	{
		strlenth=(len-strlenth)/2;
	    Show_Str(strlenth+x,y, ATK_MD0700_LCD_WIDTH, ATK_MD0700_LCD_HEIGHT, str,size,1);
	}
}   
